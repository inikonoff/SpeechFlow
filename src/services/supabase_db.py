...
class SupabaseDB:
    ...
    async def log_error(self, user_id: int, error_data: Dict[str, str]) -> bool:
        """Логирует ошибку. Если такая категория уже есть — обновляет пример."""
        try:
    ...
    # ─── Flow Deep Dive / Weekly Report ─────────────────────────────────────

    async def get_error_for_practice(self, user_id: int) -> Optional[Dict[str, Any]]:
        """Возвращает одну активную ошибку для практики."""
        try:
            response = (self.client.table("error_logs")
                        .select("*")
                        .eq("user_id", user_id)
    ...
    async def increase_error_mastery(self, error_id: int) -> int:
        try:
            res = self.client.table("error_logs").select("mastery_score").eq("id", error_id).execute()
            if not res.data:
                return 0
            current = res.data[0].get("mastery_score", 0)
            new_score = current + 1
            self.client.table("error_logs").update({"mastery_score": new_score}).eq("id", error_id).execute()
            return new_score
        except Exception as e:
    ...
    async def get_flow_errors_for_report(self, user_id: int) -> List[Dict[str, Any]]:
        try:
            ...
            for row in response.data:
                ...
                grouped[cat]["count"] += 1
            return sorted(grouped.values(), key=lambda x: x["count"], reverse=True)[:3]
        except Exception as e:
    ...
    async def clear_vocabulary(self, user_id: int) -> bool:
        try:
            self.client.table("vocabulary").delete().eq("user_id", user_id).execute()
            return True
        except Exception as e:
