    ...
    async def generate_penfriend_multibubble(
        ...
        if recasting_enabled:
            system_prompt += (
                ...
                "If the user made ANY grammatical error in their last message — you MUST recast it.\n"
                "Do not skip recasting if an error exists. Place it in whichever message fits best.\n"
                "If the user's English is perfect — no bold, respond normally."
            )
        ...
        system_prompt += (
            '\n\n# OUTPUT FORMAT\n'
            'Return a JSON object with a "messages" array. Example:\n'
            '{"messages": ["first message", "second message"]}\n'
            'Rules:\n'
            '- 2-3 items in the array\n'
            '- Each item is a plain string\n'
            '- **double asterisks** around recasted phrases are REQUIRED and must appear inside the strings\n'
            '- No markdown outside the strings, no code blocks, no preamble'
        )
        ...
        async def _multibubble(client):
            response = await client.chat.completions.create(
                ...
                temperature=0.85,
                max_tokens=300,
                # response_format намеренно убран — json_object подавляет **bold** внутри строк
            )
            return response.choices[0].message.content

        try:
            raw = await self._make_request(_multibubble)
            logger.info(f"Multibubble raw response: {raw!r}")
